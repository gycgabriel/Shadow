...
public class Flashback : Singleton<Flashback>
{
    ...
    IEnumerator FadeImage(bool fadeOut, System.Action action = null)
    {
        ...
        if (fadeOut)
        {
            ...
            for (float i = 4; i >= 0; i -= (currentTime - initTime))
            {
                Debug.Log("fading out");
                img.color = new Color(img.color[0], img.color[1], img.color[2], i * initAlpha);
        ...
        else
        {
            ...
            // loop over 4 seconds
            for (float i = 0; i <= 4; i += (Time.realtimeSinceStartup - initTime))
            {
                Debug.Log("fading in");
                img.color = new Color(img.color[0], img.color[1], img.color[2], i * initAlpha);
                currentTime = Time.realtimeSinceStartup;
